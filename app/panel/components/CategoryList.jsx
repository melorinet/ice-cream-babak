...
const CategoryList = (props) => {
  ...
  const getMenuTableData = async () => {
    ...
    try {
      const response = await fetch(
        `${BASE_URL}/api/categories/index/${props.type}`,
        {
          method: "POST",
          headers: {
  ...
  const deleteRow = async (id) => {
    ...
    try {
      const response = await fetch(
        `${BASE_URL}/api/categories/destroy/${id}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
        ...
        <div className=" min-h-[100vh] h-auto w-[75vw] pl-[2rem] pb-[10rem]   flex flex-col  ">
          ...
          <section className="bg-white min-h-[100vh] h-auto w-full mt-[2rem] rounded-[10px] pt-[2rem]   ">
            <Link
              ...
              href={`/panel/category/create/${props.type}`}
            >
              + ایجاد بخش جدید {props.type}
            </Link>
                    ...
                    return (
                      <>
                        ...
                        >
                          ...
                          <td className=" border border-gray-300">تصویر</td>
                            ...
                            {item.childs?.map((chitem, chindex) => {
                              return (
                                ...
                                >
                                  ...
                                  <td className=" border border-gray-300">
                                    <Link href={`edit/${chitem.id}`} className=" border border-gray-300 btn btn-primary">
                                      ویرایش
                                    </Link>
                                  </td>
                                  <td className=" border border-gray-300">
