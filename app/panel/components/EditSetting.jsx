...
const EditSetting = (props) => {
    ...
    try {
      if (Language === "") {
        ...
        const response = await fetch(
          `${BASE_URL}/api/settings/update/${props.id}`,
          {
            method: "POST",
            headers: {
  ...
  const getPreFilledData = async () => {
    ...
    try {
      const response = await fetch(
        `${BASE_URL}/api/settings/show/${props.id}`,
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        ...
        setsingleKey(data.data.key);
        setSettingType(data.data.type);
        if (data.data.type === "video") {
          setVideo(data.data.value.fa);
          setVideoPreview(data.data.value.fa);
        } else if (data.data.type === "text") {
          setdesc(data.data.value.fa);
        } else if (data.data.type === "image") {
          setImage(data.data.value.fa);
          setPreview(data.data.value.fa);
        }
      }
      setloading(false);
    } catch (error) {
  ...
  return (
    <section className=" w-full h-auto  flex flex-row justify-between min-h-[80vh] ">
      ...
      <div className=" min-h-[50vh] h-auto w-[75vw]  flex flex-col  ">
        <pre className=" text-zinc-700 font-semibold  ">ویرایش تنظیمات</pre>
        <div className=" flex flex-col w-full  py-[2rem] h-auto min-h-[50vh] bg-white px-[1.5rem] mt-[2rem] rounded-[10px] ">
          ...
          <section className=" w-full ml-[2rem] py-[2rem] grid grid-cols-2 gap-[1.5rem] align-middle self-center bg-white min-h-[50vh] h-auto px-[1.5rem] rounded-[10px] mt-[2rem] ">
            ...
            {/* {SettingType == "text" && SettingType !="image" && SettingType != "video" && ( */}
              <div className={` ${SettingType === "text" ? `block` : `hidden`} `} >
                <h3 className=" text-zinc-700 text-[16px] font-medium ">
                  توضیحات
                </h3>
                <textarea
                ...
                {/* <h1 className=" text-teal-600" >{SettingType}</h1> */}
            {/* {SettingType != "text" && ( */}
              <div className={`flex flex-col h-auto w-full ${SettingType !== "text" ? `flex` : `hidden` } `}>
                {/* File input */}
                ...
                <div className=" flex flex-col gap-[2rem] w-full  ">
                  <input
                    ...
                    //   accept=`SettingType === "image" ? `image/*`: SettingType === "video" ? `video/*`:""}`
                    //   accept="image/*,video/*"
