...
const EditMenu = (props) => {
  ...
  const UpdateForm = async () => {
    try {
      if (Language === "") {
        ...
        const response = await fetch(
          `${BASE_URL}/api/menus/update/${props.id}`,
          {
            method: "POST",
            headers: {
              Accept: "application/json",
  ...
  const getPreFilledData = async () => {
    ...
    try {
      const response = await fetch(
        `${BASE_URL}/api/menus/show/${props.id}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!response.ok) {
        ...
        const data = await response.json();
        console.log(data.data);
        setMotherMenu(data.data.parent_id)
        setMenuTitle(data.data.title.fa)
        setLinkType(data.data.linktype)
          ...
          <section className=" w-full grid grid-cols-2 gap-[1.5rem] align-middle self-center min-h-[50vh] h-auto   ">
            ...
            <div>
              <h3 className=" text-zinc-700 text-[16px] font-medium ">
               ...
               <select
                onChange={(e) => setMotherMenu(e.target.value)}
                value={MotherMenu}
